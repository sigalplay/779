...
export default function EveningRoutine({ mode }) {
  ...
  const childUrl = useMemo(() => (order.length && characterId ? buildChildEveningRoutineUrl(gender, characterId, order, language) : ""), [gender, characterId, order, language]);
  ...
  return (
    <AppShell mode={mode}>
      <div className="mb-6 print:hidden">
        <h1 className="font-display text-3xl font-black md:text-4xl">{t("לוח התארגנות ערב", "Evening Visual Schedule")}</h1>
        <p className="mt-1 text-muted-foreground">{t("בחרו דמות, ולאחר מכן לחצו על התמונות לפי סדר ההתארגנות המתאים למשפחה שלכם.", "Choose a character, then select pictures in the order that suits your family's routine.")}</p>
      </div>

      <div className="print:hidden mb-6 rounded-3xl border border-indigo-200 bg-indigo-50/70 p-5 md:p-6">
        <h2 className="mb-2 font-display text-base font-bold">{t("למה לוח התארגנות ערב עוזר?", "How does an evening routine visual schedule help?")}</h2>
        <p className="text-sm leading-relaxed text-foreground/90">{t("רצף חזותי קבוע עוזר לילד/ה לדעת מה השלב הבא, להתארגן באופן עצמאי ולסיים את היום ברוגע. אפשר לבחור רק את השלבים שמתאימים לכם ולסדר אותם בדיוק לפי שגרת הערב בבית.", "A consistent visual sequence helps a child know what comes next, organize more independently, and end the day calmly. Choose only the steps that suit your family and arrange them to match your evening routine.")}</p>
      </div>

      <div className="print:hidden mb-6 flex gap-2">
        {[["girl", t("👧 בת", "👧 Girl")], ["boy", t("👦 בן", "👦 Boy")]].map(([value, text]) => (
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_1.2fr] print:!grid-cols-1">
        ...
        <section className="routine-print-sheet rounded-3xl border border-border/60 bg-card p-5 md:p-6 print:!rounded-none print:!border-none print:!p-0">
          <div className="mb-4 flex items-center justify-between print:hidden"><h2 className="font-display text-lg font-bold">{t("הלוח שלנו", "Our visual schedule")}</h2>{scheduled.length > 0 && <Button variant="ghost" size="sm" onClick={() => setOrder([])} className="rounded-full text-muted-foreground"><RotateCcw className="h-3.5 w-3.5" />{" "}{t("איפוס", "Reset")}</Button>}</div>
          ...
          {scheduled.length > 0 && <div className="mt-5 space-y-2 print:hidden"><Button onClick={() => setShareOpen(true)} variant="outline" className="w-full rounded-full"><Smartphone className="h-4 w-4" />{" "}{t("קישור לילד/ה - סימון בפלאפון", "Child link — tick off on a phone")}</Button><Button onClick={() => window.print()} className="w-full rounded-full"><Printer className="h-4 w-4" />{" "}{t("הדפסה / שמירה כ-PDF", "Print / Save as PDF")}</Button></div>}
        </section>
      </div>

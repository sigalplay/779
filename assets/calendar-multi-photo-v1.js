(()=>{
 ...
 function notify(message){const box=document.createElement('div');box.className='calendar-photo-notice';box.setAttribute('role','status');box.textContent=message;document.body.append(box);setTimeout(()=>box.remove(),2600)}
  function controls(input){let box=document.querySelector('.calendar-multi-photo-controls');if(!box){box=document.createElement('div');box.className='calendar-multi-photo-controls';box.innerHTML='<span class="calendar-multi-photo-count"></span><button type="button" data-photo-remove></button><button type="button" data-photo-reset></button>';input.closest('label')?.after(box);box.querySelector('[data-photo-remove]').addEventListener('click',()=>{if(!selected)return;months.set(activeKey,current().filter(x=>x.id!==selected));selected='';render()});box.querySelector('[data-photo-reset]').addEventListener('click',()=>{current().forEach((p,i)=>Object.assign(p,initialBox(p,i)));render()})} const list=current();box.querySelector('.calendar-multi-photo-count').textContent=t.count(list.length);const del=box.querySelector('[data-photo-remove]');del.textContent=t.remove;del.disabled=!selected;box.querySelector('[data-photo-reset]').textContent=t.reset;}
 function read(file){return new Promise(resolve=>{const r=new FileReader();r.onload=()=>{if(typeof r.result!=='string'){resolve(null);return}const img=new Image();img.onload=()=>resolve({src:r.result,width:img.naturalWidth,height:img.naturalHeight});img.onerror=()=>resolve(null);img.src=r.result};r.onerror=()=>resolve(null);r.readAsDataURL(file)})}
 function initialBox(photo,index){const ratio=(photo.naturalWidth||1)/Math.max(photo.naturalHeight||1,1);let w=30,h=30;if(ratio>1.12){w=43;h=27}else if(ratio<.89){w=25;h=43}const positions=[[4,6],[53,6],[4,53],[53,53],[29,29]],position=positions[index]||positions[0];return{x:Math.min(position[0],100-w),y:Math.min(position[1],100-h),w,h}}
 async function pick(e){const files=[...(e.target.files||[])];if(!files.length)return;const valid=files.filter(f=>f.type.startsWith('image/'));if(valid.length!==files.length)notify(t.bad);const list=current(),room=5-list.length;if(room<=0){notify(t.limit);e.target.value='';return}if(valid.length>room)notify(t.limit);const data=await Promise.all(valid.slice(0,room).map(read));data.filter(Boolean).forEach(photo=>{const stored={id:uid(),src:photo.src,naturalWidth:photo.width,naturalHeight:photo.height};list.push({...stored,...initialBox(stored,list.length)})});selected=list.at(-1)?.id||'';e.target.value='';render()}
 ...
 function render(){if(gesture)return;const input=uploadInput();if(!input)return;input.multiple=true;input.accept='image/png,image/jpeg,image/webp,image/gif';const label=input.closest('label');if(label){[...label.childNodes].filter(n=>n.nodeType===3&&n.textContent.trim()).forEach(n=>n.textContent=` ${t.upload}`)}controls(input);document.querySelectorAll('.calendar-photo-wrap').forEach((wrap,wi)=>{const sheet=wrap.closest('.hebrew-calendar-sheet'),sheetKey=sheet?.querySelector('.calendar-sheet-title')?.textContent?.trim()||activeKey,isPrintCopy=Boolean(wrap.closest('.calendar-print-pages')),list=isPrintCopy?(months.get(activeKey)||[]):(months.get(sheetKey)||[]),signature=JSON.stringify([sheetKey,list.map(p=>[p.id,p.x,p.y,p.w,p.h]),wi===0?selected:'']);let stage=wrap.querySelector('.calendar-multi-photo-stage');if(!list.length){stage?.remove();return}if(stage?.dataset.signature===signature)return;if(!stage){stage=document.createElement('div');stage.className='calendar-multi-photo-stage';wrap.append(stage)}stage.dataset.signature=signature;stage.replaceChildren();list.forEach((p,i)=>{const item=document.createElement('div');item.className=`calendar-multi-photo-item${p.id===selected&&wi===0?' is-selected':''}`;item.style.cssText=`left:${p.x}%;top:${p.y}%;width:${p.w}%;height:${p.h}%`;item.innerHTML=`<img alt="${t.photo(i+1)}"><span class="calendar-multi-photo-resize" aria-label="${isEnglish?'Resize photo':'שינוי גודל התמונה'}"></span>`;item.querySelector('img').src=p.src;stage.append(item);if(wi===0)bind(item,p,stage)})})}

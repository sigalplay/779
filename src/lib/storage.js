...
export function getLocalSavedContent() {
  return { favorites: read(KEYS.favorites, []), folders: read(KEYS.folders, []), plans: read(KEYS.plans, []) };
}

export function replaceLocalSavedContent({ favorites = [], folders = [], plans = [] }) {
...
function syncInBackground(action) {
  import("./cloud-data").then(action).catch(() => window.dispatchEvent(new Event("pp_cloud_sync_error")));
}
...
export function allActivities() {
  return [...customActivities(), ...getCachedCmsCollection("activity", SEED_ACTIVITIES)]
    .map((activity) => {
      const withoutVisual = (items) => Array.isArray(items) ? items.filter((item) => item !== "חזותית") : items;
      const hasPaperTowel = activity.materials?.some((item) => String(item).includes("נייר סופג"));
      const goals = withoutVisual(activity.goals) || [];
      return {
        ...activity,
        categories: withoutVisual(activity.categories),
        goals: hasPaperTowel && !goals.includes("ויסות כוח") ? [...goals, "ויסות כוח"] : goals,
        functions: withoutVisual(activity.functions),
        sensory_systems: withoutVisual(activity.sensory_systems),
        tags: withoutVisual(activity.tags),
      };
    })
    .sort(newestActivitiesFirst);
}
...
export function searchActivities(params) {
  ...
  if (params.audience) {
    // Therapists use the complete activity bank, including activities that were
    // originally published for parents. Parent searches keep their audience filter.
    if (params.audience !== "therapist") {
...
export function buildTreatmentPlan(params) {
  const pool = allActivities().filter(
    (a) => isSearchActive(a) && a.age_min <= params.age && a.age_max >= params.age,
  );
...
export function saveTreatmentPlan(title, items, params) {
  ...
  syncInBackground(({ savePlanToCloud }) => savePlanToCloud(plan));
  return plan;
}
...
export function updateTreatmentPlan(id, title, items, params) {
  ...
  list[index] = updated;
  write(KEYS.plans, list);
  syncInBackground(({ savePlanToCloud }) => savePlanToCloud(updated));
  return updated;
}
...
export function deleteTreatmentPlan(id) {
  ...
  syncInBackground(({ deletePlanFromCloud }) => deletePlanFromCloud(id));
}
...
export function createFolder(name) {
  ...
  syncInBackground(({ saveFolderToCloud }) => saveFolderToCloud(folder));
  return folder;
}
...
export function toggleFavorite(activityId, folderId = null) {
  ...
  if (idx >= 0) {
    ...
    syncInBackground(({ deleteFavoriteFromCloud }) => deleteFavoriteFromCloud(removed.id));
  ...
  write(KEYS.favorites, favs);
  syncInBackground(({ saveFavoriteToCloud }) => saveFavoriteToCloud(favorite));
  return { favored: true };
}

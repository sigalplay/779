...
export default function SharedHebrewCalendar() {
  ...
  useEffect(() => {
    const clearMobilePrint = () => {
      document.body.classList.remove("calendar-mobile-print");
      document.body.classList.remove("calendar-printing");
    };
    window.addEventListener("afterprint", clearMobilePrint);
    return () => {
      clearMobilePrint();
      window.removeEventListener("afterprint", clearMobilePrint);
    };
  }, []);
  ...
  return <main className="shared-hebrew-calendar bg-sky/15 p-4 md:p-8" dir="rtl"><div className="mx-auto mb-5 flex max-w-5xl justify-between print:hidden"><Link to="/"><Button variant="outline"><Home className="h-4 w-4" /> בואו נשחק</Button></Link><Button onClick={printCalendar}><Printer className="h-4 w-4" /> הדפסה</Button></div><div className="mx-auto max-w-5xl"><HebrewCalendarPages months={months} year={data.year} settings={data.settings} title={data.title} customEvents={data.customEvents || []} showAll /></div></main>;
}

...
import { refreshPublishedCmsContent } from "@/lib/cms-content";
...
export default function TherapistBuild() {
  ...
  useEffect(() => {
    const refreshActivities = () => setCmsRevision((value) => value + 1);
    window.addEventListener("boo_cms_change", refreshActivities);
    refreshPublishedCmsContent().catch(() => null);
    return () => window.removeEventListener("boo_cms_change", refreshActivities);
  }, []);
  ...
  const candidates = useMemo(() => {
    ...
    const pool = allActivities().filter(isSearchActive);
  ...
  }, [goals, durationMode, contentType, cmsRevision]);
  ...
  const craftActivities = useMemo(
    () => allActivities().filter((a) => isSearchActive(a) && a.tags?.includes("יצירה")),
    [cmsRevision],
  );
  ...
  const therapistActivities = useMemo(
    () => allActivities().filter(isSearchActive),
    [cmsRevision],
  );

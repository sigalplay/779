...
import { DAILY_SEQUENCE_CATEGORIES, DAILY_SEQUENCE_TEMPLATES, stepsForDailySequence } from "@/lib/daily-sequences";
...
export default function DailySequences() {
  ...
  const category = DAILY_SEQUENCE_CATEGORIES.find((item) => item.id === categoryId);
  const visibleTemplates = categoryId
    ? DAILY_SEQUENCE_TEMPLATES.filter((item) => !item.hidden && item.category === categoryId)
    : DAILY_SEQUENCE_TEMPLATES.filter((item) => !item.hidden && !item.category);
  ...
  if (!templateId) return (
    <AppShell mode="parent">
      <div className="mb-7">
        {categoryId ? <button onClick={() => setCategoryId(null)} className="mb-2 inline-flex items-center gap-1 text-sm font-bold text-muted-foreground"><ArrowRight className="h-4 w-4" /> חזרה לכל הרצפים</button> : null}
        <h1 className="font-display text-4xl font-black">{category?.title || "רצפים של פעולות יום־יומיות"}</h1>
        <p className="mt-2 text-muted-foreground">{category ? "בחרו רצף בתחום הלבוש." : "בחרו רצף, סדרו את השלבים והדפיסו לוח חזותי ברור."}</p>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        {!categoryId && DAILY_SEQUENCE_CATEGORIES.filter((item) => !item.hidden).map((item) => (
          <button key={item.id} type="button" onClick={() => setCategoryId(item.id)} className="group overflow-hidden rounded-3xl border border-sky/60 bg-white text-right shadow-sm transition hover:-translate-y-1 hover:shadow-md">
  ...
  return (
    <AppShell mode="parent">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 print:hidden"><div><button onClick={() => setTemplateId(null)} className="mb-2 inline-flex items-center gap-1 text-sm font-bold text-muted-foreground"><ArrowRight className="h-4 w-4" /> {categoryId ? `חזרה ל${category?.title}` : "חזרה לכל הרצפים"}</button><h1 className="font-display text-3xl font-black">{template?.title}</h1></div><div className="flex gap-2"><Button variant="outline" className="rounded-full" onClick={() => { setOrder(allSteps.filter((step) => !step.optional).map((step) => step.id)); setCompleted(new Set()); }}><RotateCcw className="h-4 w-4" /> איפוס</Button><Button className="rounded-full" onClick={() => window.print()}><Printer className="h-4 w-4" /> הדפסה</Button></div></div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 print:hidden"><div className="flex gap-2">{!template?.genderNeutral ? <><button type="button" onClick={() => setGender("girl")} className={`rounded-full border px-5 py-2 text-sm font-bold ${gender === "girl" ? "border-primary bg-primary text-primary-foreground" : "bg-white"}`}>בת</button><button type="button" onClick={() => setGender("boy")} className={`rounded-full border px-5 py-2 text-sm font-bold ${gender === "boy" ? "border-primary bg-primary text-primary-foreground" : "bg-white"}`}>בן</button></> : null}</div><div className="flex gap-2"><button type="button" onClick={() => setLayout("grid")} className={`rounded-full border px-4 py-2 text-sm font-bold ${layout === "grid" ? "border-sky bg-sky/30" : "bg-white"}`}>כרטיסים</button><button type="button" onClick={() => setLayout("column")} className={`rounded-full border px-4 py-2 text-sm font-bold ${layout === "column" ? "border-sky bg-sky/30" : "bg-white"}`}>רצף לאורך</button></div></div>

...
import { preparePhotoForCropLocally, removePhotoBackgroundLocally } from "@/lib/local-photo-cutout";
import { useTranslator } from "@/lib/language";
import { useCmsCollection } from "@/lib/cms-content";

const GENERAL_SOCIAL_STORY_IDS = new Set(["losing-game", "not-getting-want", "personal-space"]);
  ...
  return (
    <section className="rounded-3xl border border-sage/40 bg-sage/10 p-4 print:hidden">
      <h3 className="mb-1 text-center font-display text-lg font-black">
        {endingsOnly ? story.endingPrompt : story.branchPrompt}
      </h3>
...
function interactiveStoryConfig(templateId, gender, language) {
  ...
  const configs = {
    "losing-game": {
      ...
      endingOptions: [
        ["winner-walks-away", he ? (isGirl ? "הילדה שניצחה מתרחקת" : "הילד שניצח מתרחק") : "The winner walks away", `${base}/ending-winner-walks-away.png`],
        ["different-game", he ? (isGirl ? "הן משחקות במשחק אחר" : "הם משחקים במשחק אחר") : "They play a different game", `${base}/ending-different-game.png`],
      ],
    },
    "waiting-turn": {
...
export default function SocialStories({ mode }) {
  ...
  function handleTemplateSelect(template) {
    ...
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => storyOptionsRef.current?.scrollIntoView({ behavior, block: "start" }));
    });
  }
  ...
  async function handlePhotoUpload(e) {
    ...
    try {
      ...
      try {
        const prepared = await preparePhotoForCropLocally(file);
        setCropRequest({ source: prepared, role: "child", label: gender === "girl" ? "הילדה" : "הילד" });
        toast.info("התמונה התקבלה. אפשר למקם ולחתוך אותה ידנית.");
      } catch {
  ...
  async function handleMotherPhotoUpload(e) {
    ...
    try {
      ...
      try {
        const prepared = await preparePhotoForCropLocally(file);
        setCropRequest({ source: prepared, role: "mother", label: "האמא" });
        toast.info("התמונה התקבלה. אפשר למקם ולחתוך אותה ידנית.");
      } catch {
        toast.error("לא הצלחנו לפתוח את התמונה. נסו תמונת JPG או PNG אחרת.");
      }
  ...
  function handleGenerate() {
    ...
    const interactiveConfig = interactiveStoryConfig(templateId, selectedGender, language);
    if (interactiveConfig) {
      result = {
        ...result,
        wordless: true,
        interactive: true,
        branchPrompt: interactiveConfig.branchPrompt,
        endingPrompt: interactiveConfig.endingPrompt,
        choiceInstructions: interactiveConfig.choiceInstructions,
        // Cover + two fixed scenes form the opening; the family chooses what follows.
        pages: result.pages.slice(0, 2),
        branchOptions: interactiveConfig.branchOptions,
        endingOptions: interactiveConfig.endingOptions,
      };
    }
    const coverPage = {
      id: uid(),
    ...
    setStory({ id: uid(), title: result.title, gender: selectedGender, templateId, kindergartenRest, illustrationStyle, wordless: !!result.wordless, interactive: !!result.interactive, branchPrompt: result.branchPrompt, endingPrompt: result.endingPrompt, choiceInstructions: result.choiceInstructions, branchOptions, endingOptions, pages: [coverPage, ...contentPages], childPhoto: result.personalized === false ? null : childPhoto, motherPhoto: result.personalized === false ? null : motherPhoto });
    setStoryMode("edit");
    setViewPage(0);
  ...
  function chooseStoryEnding(option) {
  ...
  function handleOpenFromLibrary(s) {
    const savedGender = s.gender || "girl";
    const fallbackInteractive = s.interactive ? interactiveStoryConfig(s.templateId, savedGender, language) : null;
    setStory({
      ...s,
      gender: savedGender,
      wordless: !!s.wordless,
      branchPrompt: s.branchPrompt || fallbackInteractive?.branchPrompt,
      endingPrompt: s.endingPrompt || fallbackInteractive?.endingPrompt,
      choiceInstructions: s.choiceInstructions || fallbackInteractive?.choiceInstructions,
      pages: s.pages.map((p) => ({ ...p, id: p.id ?? uid() })),
        ...
        <>
          {!story && (
            <div className="space-y-5 rounded-3xl border border-border/60 bg-card p-6 print:hidden">
              ...
              <div ref={storyOptionsRef} className="scroll-mt-24" aria-hidden="true" />
            ...
            <div>
              ...
              {storyMode === "view" ? (
                <div className="print:hidden">
                  ...
                  {story.interactive && viewPage === 2 && (
                    <div className="mx-auto mt-5 max-w-4xl"><InteractiveStoryChoices story={story} onBranch={chooseStoryBranch} /></div>
                  )}
                  {story.interactive && viewPage === 3 && story.interactiveChoice && (
                    <div className="mx-auto mt-5 max-w-3xl"><InteractiveStoryChoices story={story} onEnding={chooseStoryEnding} endingsOnly /></div>
                  )}
              ...
              <div className="space-y-6">
                {story.pages.map((page, i) => (
                  ...
                  {story.interactive && i === 3 && <InteractiveStoryChoices story={story} onBranch={chooseStoryBranch} />}
                  ...
                  {story.interactive && i === 3 && <InteractiveStoryChoices story={story} onEnding={chooseStoryEnding} endingsOnly />}
                  </div>
                ))}
                {story.interactive && story.pages.length === 3 && <InteractiveStoryChoices story={story} onBranch={chooseStoryBranch} />}
              </div>

      ...
      >
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            {subtitle ? <p className={cn("mt-1 font-display font-bold text-foreground", large ? "text-lg md:text-xl" : "text-sm")}>{subtitle}</p> : null}
...
export function IllustratedNavButton({ onClick, image, title, subtitle, description, large = false, className, captionClassName }) {
  const { language } = useTranslator();
  return (
    <button
      type="button"
    ...
    >
      <CardContent image={image} title={title} subtitle={subtitle} description={description} large={large} showArrow captionClassName={captionClassName} language={language} />
    </button>
  );
}

    ...
    <section className={cn("hebrew-calendar-sheet bg-white", !isLast && "hebrew-calendar-break")} dir="rtl">
      ...
      <div className="calendar-grid" style={{ "--calendar-week-rows": weekRows }}>
        ...
        <svg
          ...
          viewBox={`0 0 700 ${(weekRows + 1) * 100}`}
          preserveAspectRatio="none"
          aria-hidden="true"
        >
          <rect x="0.5" y="0.5" width="699" height={(weekRows + 1) * 100 - 1} vectorEffect="non-scaling-stroke" />
          {[1, 2, 3, 4, 5, 6].map((column) => (
            <line key={`v-${column}`} x1={column * 100} y1="0" x2={column * 100} y2={(weekRows + 1) * 100} vectorEffect="non-scaling-stroke" />
          ))}
          {Array.from({ length: weekRows }, (_, index) => index + 1).map((row) => (
            <line key={`h-${row}`} x1="0" y1={row * 100} x2="700" y2={row * 100} vectorEffect="non-scaling-stroke" />
          ))}
        </svg>
      </div>

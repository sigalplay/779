...
export function TherapistPostureScissorsTips({ children = null, showScissors = true }) {
  ...
  // The guidance icons are intentionally visible whenever a page is opened.
  // Hiding them is temporary for the current page only, so they never remain
  // unexpectedly hidden in another activity or on a future visit.
  ...
  function toggleIcons() {
    setIconsHidden((current) => !current);
    setOpenPanel(null);
  }
  ...
  return (
    <div className="fixed left-4 top-[calc(50%-4rem)] z-40 flex flex-col gap-3 print:hidden">
      <button
        type="button"
        onClick={toggleIcons}
        aria-label={iconsHidden ? "הצגת סמלי העזר" : "הסתרת סמלי העזר"}
        aria-expanded={!iconsHidden}
        title={iconsHidden ? "הצגת סמלי העזר" : "הסתרת סמלי העזר"}
        className="flex h-11 w-11 items-center justify-center self-center rounded-full border border-border/60 bg-white text-muted-foreground shadow-md transition-colors hover:bg-muted"
      >
        {iconsHidden ? <Eye className="h-5 w-5" /> : <EyeOff className="h-5 w-5" />}
      </button>

      <div className={iconsHidden ? "hidden" : "contents"}>
